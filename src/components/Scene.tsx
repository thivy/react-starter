...
export default function Scene(props: SceneProps) {
  ...
  useFrame(({ clock, camera }, delta) => {
    ...
    const viewOffset =
      size.width < 650
        ? size.height * (size.width <= 360 ? 0.04 : 0.02)
        : size.height < 500
          ? size.height * 0.16
          : 0;
    if (viewOffset > 0) {
      if (
        !perspective.view?.enabled ||
        perspective.view.offsetY !== viewOffset ||
        perspective.view.fullWidth !== size.width ||
        perspective.view.fullHeight !== size.height
      ) {
        perspective.setViewOffset(size.width, size.height, 0, viewOffset, size.width, size.height);
      }
